...
def analyze_csv(input_csv, output_csv):
    ...
    with open(input_csv, 'r', encoding='utf-8') as infile:
        ...
        for row in reader:
          ...
          category_numbers = extract_numbers(row['Column5'])  
            ...
            # Check if any number in the extracted list matches filter_list
          if any(num in filter_list for num in category_numbers):
            total_filtered_domains += 1  
            https_status = row['https_status']
            if "service.tim.it" in https_status:
                ssl_data.append(row)
            elif "timed out." in https_status:
                ...
                expired.append(row)
            elif "self signed certificate" in https_status:
                ...
                #grouped_data[https_status].append(row)
    ...
    grouped_data["certificate"] = certificate
...
#only for dns for family dataset  
